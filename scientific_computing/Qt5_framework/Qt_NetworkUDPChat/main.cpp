...
int main(int argc, char *argv[])
{
    ...
    // stdlin is a file handle;
    CCommandLine command_line(&a, stdin);
    QFuture<void> future = QtConcurrent::run(&command_line, &CCommandLine::monitor);

    CChat chat;

    // Connetct signal CComandLine::command (command); consuming slot is function CChat::command (chat); mode QueuedConnection;
    QObject::connect(&command_line,&CCommandLine::command,&chat,&CChat::command,Qt::QueuedConnection);
