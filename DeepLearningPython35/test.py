...
#component testing
training_data_comp = training_data
test_data_comp = test_data

component_testing_on = 0
if (component_testing_on):
    print(len(training_data))
    print(len(training_data[0]))
    print(training_data[0][0].shape)
    print(training_data[0][1].shape)
    print(len(training_data_comp))
    print(len(training_data_comp[0]))
    print(training_data_comp[0][0].shape)
    print(training_data_comp[0][1].shape)

    for i in range(len(test_data_comp)):
        for j in range(len(test_data_comp[i])):
            if j==0:
                for k in range(len(test_data_comp[i][j])):
                    test_data_comp[i][j][k] = 0.7
            else:
                print(test_data_comp[i][j])

    for i in range(len(training_data_comp)):
        print(i)
        for j in range(len(training_data_comp[i])):
            #print(len(training_data1[j]))
            for k in range(len(training_data_comp[i][j])):
                training_data_comp[i][j][k] = 0.52
...
net = network.Network([784, 30, 10], component_testing_on)
#net = network.Network([784, 50, 30, 10], component_testing_on)
net.SGD(training_data, 30, 10, 3.0, component_testing_on, test_data=test_data)
...
aMatrix = training_data[0].get_value()
import theano.tensor as T
bVector = training_data[1].eval()
